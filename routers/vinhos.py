...
router = APIRouter(prefix="/vinhos", tags=["vinhos"])
...
# ------------------ POST ------------------
@router.post("/", response_model=schemas.VinhoResponse)
def criar_vinho(vinho: schemas.VinhoCreate, db: Session = Depends(get_db)):
    db_vinho = models.Vinho(
        ...
        imagem=vinho.imagem  # <-- adiciona aqui
    )
    db.add(db_vinho)
    db.commit()
    db.refresh(db_vinho)
...
# ------------------ GET ------------------
@router.get("/", response_model=list[schemas.VinhoResponse])
def listar_vinhos(db: Session = Depends(get_db)):
    return db.query(models.Vinho).all()


@router.get("/{vinho_id}", response_model=schemas.VinhoResponse)
...
# ------------------ PUT ------------------
@router.put("/{vinho_id}", response_model=schemas.VinhoResponse)
def atualizar_vinho(vinho_id: int, vinho: schemas.VinhoCreate, db: Session = Depends(get_db)):
    ...
    db_vinho.imagem = vinho.imagem  # <-- adiciona aqui
